...
1. CART ARCHITECTURE - APP ROUTER MIGRATION
tRPC Approach (v1) vs Server Actions Approach (v2)
Aspetto	tRPC (Catalogo v1)	Server Actions (Catalogo v2)
Routing	API routes separate (/api/trpc/[trpc])	Actions integrate nei componenti/server
Data Fetching	useQuery/useMutation client-side	useActionState, useFormStatus
Type Safety	✅ Automatica via inferRouterInputs	✅ Manuale con Zod schemas
RPC Pattern	✅ Chiamate esplicite tipo funzione	❌ Nessun endpoint RPC, solo form actions
File Structure	src/server/trpc/routers/	src/server/actions/
Middleware	tRPC middleware (logging, auth)	Next.js middleware + Server Action wrappers
Realtime Updates	❌ Richiede polling/WebSockets	❌ Stesso limite
Bundle Size	⚠️ Client + server runtime	✅ Solo server runtime nel client
Migration Path: File Replacement Matrix

✅ DA MANTENERE (Business Logic)

src/lib/prisma.ts (Prisma client)

src/types/cart.ts (TypeScript interfaces)

src/components/ui/ (UI components non legati a tRPC)

❌ DA SOSTITUIRE

src/server/trpc/routers/cart.ts → src/server/actions/cart-actions.ts

src/server/trpc/routers/checkout.ts → src/server/actions/checkout-actions.ts

src/pages/cart.tsx → app/cart/page.tsx

src/pages/checkout/[...].tsx → app/checkout/[...]/page.tsx

⚠️ DA AGGIORNARE

Componenti che usano trpc.useQuery() → fetch() o Server Components

Context providers che dipendono da tRPC → React cache() e useOptimistic

Route Structure per App Router
text
Copia
Scarica
app/
...
│   └── actions/                # Client-side cart actions
│       └── add-to-cart.tsx     # Client component con useActionState
├── checkout/
│   ├── layout.tsx              # Shared checkout layout con progress
...
└── actions/
    └── cart/
        └── route.ts            # API route per backward compatibility
typescript:src/app/layout.tsx
Copia
Scarica
// Root layout - Setup per cart persistence
...
export default async function RootLayout({
  ...
  return (
    <html lang="it">
      <body>
        <CartProvider initialGuestCartId={guestCartId}>
          {children}
        </CartProvider>
      </body>
    </html>
...
typescript:src/server/actions/cart-actions.ts
Copia
Scarica
'use server';
...
// Zod schemas per input validation
...
// Utility: Ottieni o crea cart ID
async function getOrCreateCartId(): Promise<string> {
  const user = await currentUser();
  
  if (user) {
    // Cart utente autenticato
...
// Action 1: Aggiungi prodotto al carrello
export async function addToCartAction(
  ...
  try {
    ...
    const product = await prisma.product.findUnique({
      ...
      include: { variants: validatedData.variantId ? true : false },
    });
    ...
    if (!product.isActive) {
      return { success: false, error: 'Prodotto non disponibile' };
    }

    // Verifica stock per variant
    if (validatedData.variantId) {
      ...
      if (!variant || variant.stock < validatedData.quantity) {
        return { 
          success: false, 
          error: `Stock insufficiente. Disponibili: ${variant?.stock || 0}` 
        };
      }
    } else if (product.stock < validatedData.quantity) {
      return { 
        success: false, 
        error: `Stock insufficiente. Disponibili: ${product.stock}` 
      };
    }
    ...
    if (existingItem) {
      ...
      await prisma.cartItem.create({
        data: {
          ...
          price: product.price, // Prezzo al momento dell'aggiunta
        },
      });
    }
    ...
    return { 
      success: true, 
      message: `${validatedData.quantity}x ${product.name} aggiunto al carrello` 
    };
  } catch (error) {
    if (error instanceof z.ZodError) {
      return { 
        success: false, 
        error: `Validazione fallita: ${error.errors[0].message}` 
      };
    }
...
export async function updateQuantityAction(
  ...
  try {
    ...
    if (!cartItem) {
      return { success: false, error: 'Item non trovato nel carrello' };
    }
    ...
    if (availableStock === undefined || availableStock < validatedData.quantity) {
      return { 
        success: false, 
        error: `Stock insufficiente. Disponibili: ${availableStock}` 
      };
    }
...
// Action 3: Rimuovi item dal carrello
...
export async function clearCartAction(): Promise<{ 
  ...
  try {
    ...
    return { success: false, error: 'Errore durante lo svuotamento' };
  }
}

// Action 5: Ottieni carrello corrente
...
typescript:src/server/actions/checkout-actions.ts
Copia
Scarica
'use server';
...
// Action 1: Valida carrello prima del checkout
export async function validateCheckoutAction(): Promise<{
  valid: boolean;
  errors: string[];
  cartTotal: number;
  items: Array<{
  ...
  try {
    ...
    // Validazione sincrona di ogni item
    for (const item of cart.items) {
      // Verifica stock attuale
      ...
      if (currentStock === undefined || currentStock < item.quantity) {
        errors.push(
          `${item.product.name}: Stock insufficiente. Disponibili: ${currentStock || 0}`
        );
      }

      // Verifica prodotto attivo
      if (!item.product.isActive) {
        errors.push(`${item.product.name}: Prodotto non più disponibile`);
      }
    ...
    return {
      valid: false,
      errors: ['Errore durante la validazione del carrello'],
      cartTotal: 0,
      items: [],
    };
  }
  ...
  try {
    ...
    if (coupon.maxUses && coupon.usedCount >= coupon.maxUses) {
      return {
        ...
        discountAmount: 0,
        discountType: 'fixed',
        error: 'Coupon esaurito',
      };
    }

    // Verifica data validità
    if (coupon.startsAt && coupon.startsAt > new Date()) {
      return {
        valid: false,
        discountAmount: 0,
        discountType: 'fixed',
        error: 'Coupon non ancora valido',
      };
    }

    const cart = await validateCheckoutAction();
    if (!cart.valid) {
      return {
        valid: false,
        discountAmount: 0,
    ...
    return {
      ...
      message: `Coupon applicato: -${discountAmount.toFixed(2)}€`,
    };
  } catch (error) {
    ...
    return {
      ...
      discountAmount: 0,
      discountType: 'fixed',
      error: 'Errore durante l\'applicazione del coupon',
    };
  }
}

// Action 3: Crea ordine con transaction Prisma
export async function createOrderAction(
  shippingData: z.infer<typeof ShippingSchema>,
  ...
  try {
    // Inizia transaction Prisma
    transaction = await prisma.$transaction(async (tx) => {
      const user = await currentUser();
      
      // 1. Recupera e valida carrello con lock
      ...
      // 2. Applica optimistic lock per stock
      const orderItems = [];
      let orderTotal = 0;
      ...
      for (const item of cart.items) {
        const currentStock = item.variantId
          ...
          : { stock: item.product.stock, price: item.product.price };

        if (!currentStock || currentStock.stock < item.quantity) {
          throw new Error(
            `Stock insufficiente per ${item.product.name}. Disponibili: ${currentStock?.stock || 0}`
          );
        }
      ...
      // 5. Aggiorna carrello come COMPLETED
      await tx.cart.update({
        where: { id: cart.id },
        data: { status: 'COMPLETED' },
      });

      // 6. Crea session Stripe se pagamento con carta
      let stripeSessionUrl = undefined;
      ...
      if (paymentMethod === 'card' && stripe) {
        const session = await stripe.checkout.sessions.create({
          ...
          mode: 'payment',
          success_url: `${process.env.NEXT_PUBLIC_APP_URL}/checkout/confirmation?orderId=${order.id}&session_id={CHECKOUT_SESSION_ID}`,
    ...
    // 7. Redirect in base al payment method
    if (paymentMethod === 'card' && transaction.stripeSessionUrl) {
      redirect(transaction.stripeSessionUrl);
    } else {
      redirect(`/checkout/confirmation?orderId=${transaction.order.id}`);
    }

    return { 
      success: true, 
      orderId: transaction.order.id,
      stripeSessionUrl: transaction.stripeSessionUrl,
    };
  } catch (error) {
...
4. CART UI WITH OPTIMISTIC UPDATES
typescript:src/components/cart/cart-drawer.tsx
Copia
Scarica
'use client';

import { useActionState, useOptimistic, useFormStatus } from 'react';
import { addToCartAction, updateQuantityAction, removeFromCartAction } from '@/server/actions/cart-actions';
...
export function CartDrawer({ initialCart }: CartDrawerProps) {
  const [optimisticCart, setOptimisticCart] = useOptimistic<OptimisticCart>(
    { items: initialCart?.items || [] }
  );

  // Add to cart con optimistic UI
  const [addToCartState, addToCartDispatch] = useActionState(
    addToCartAction,
    { success: false }
  );

  // Add to cart handler con optimistic update
  const handleAddToCart = async (formData: FormData) => {
    ...
    await addToCartDispatch(formData);
  };

  // Update quantity con optimistic UI e rollback
  const handleUpdateQuantity = async (cartItemId: string, newQuantity: number) => {
    const currentItem = optimisticCart.items.find(item => item.id === cartItemId);
    ...
    // Server action
    const result = await updateQuantityAction(cartItemId, newQuantity);
  ...
  // Remove item con optimistic UI
  const handleRemoveItem = async (cartItemId: string) => {
    const itemToRemove = optimisticCart.items.find(item => item.id === cartItemId);
    
    // Optimistic update
    setOptimisticCart((current) => ({
      items: current.items.filter(item => item.id !== cartItemId),
      isOptimistic: true,
    }));
    ...
    if (!result.success && itemToRemove) {
      setOptimisticCart((current) => ({
        items: [...current.items, itemToRemove],
        isOptimistic: false,
      }));
    }
  };
  ...
  return (
    <div className="cart-drawer">
      <div className="cart-header">
        <h2>Carrello</h2>
        <CartCountBadge count={optimisticCart.items.length} />
      </div>
        ...
        <div className="cart-items">
          {optimisticCart.items.map((item) => (
            <CartItem
              ...
              isOptimistic={item.id.startsWith('optimistic-')}
            />
          ))}
        </div>
      )}
      ...
      <CartTotal items={optimisticCart.items} />
    </div>
  );
}

// Componente per badge contatore carrello
...
// Componente singolo item con form status
function CartItem({ 
  item, 
  onUpdateQuantity, 
  onRemove,
  ...
  return (
    <div className={`cart-item ${isOptimistic ? 'opacity-50' : ''}`}>
      <div className="item-info">
        <h3>{item.product.name}</h3>
        {item.variant && <p>Variante: {item.variant.name}</p>}
...
// Selectore quantità con form status
  ...
  return (
    <div className="quantity-selector">
      <button
        ...
        disabled={quantity <= 1 || disabled || pending}
      >
        -
      </button>
      <span>{quantity}</span>
...
// Totale carrello
function CartTotal({ items }: { items: OptimisticCart['items'] }) {
...
5. CHECKOUT FLOW - APP ROUTER
typescript:src/app/checkout/layout.tsx
Copia
Scarica
  ...
  return (
    <div className="checkout-layout">
      ...
      <footer className="checkout-footer">
        <p>Pagamento sicuro • 30 giorni per il reso • Assistenza 24/7</p>
      </footer>
    </div>
  );
}
typescript:src/components/checkout/checkout-progress.tsx
Copia
Scarica
'use client';
...
const steps = [
  { id: 'cart', label: 'Carrello', path: '/checkout' },
  { id: 'shipping', label: 'Spedizione', path: '/checkout/shipping' },
  { id: 'payment', label: 'Pagamento', path: '/checkout/payment' },
  { id: 'confirmation', label: 'Conferma', path: '/checkout/confirmation' },
];
          ...
          return (
            <li
              ...
              className={`
                ...
                ${isCurrent ? 'current' : ''}
                ${index > currentStepIndex ? 'upcoming' : ''}
              `}
            >
              {isCompleted || isCurrent ? (
                ...
                <div className="step-inactive">
                  <span className="step-number">{index + 1}</
...
x.order.update({
      ...
      // Aggiorna stock definitivamente
